...
lTab_Science = [
  ...
  [
    ...
    sg.Button('Cancel_Science', key='-Cancel_Science-', visible=False)
  ]
]
...
window = sg.Window("Game", LAYOUT, size=(600, 400), resizable=True)
fTicks = 0
iCredits = 500
...
def do_research(sCurrent_Research):
  ...
  iMax_Science = SCIENCE[sCurrent_Research]['duration']
  iCurrent_Value = 0
...
while True:
  ...
  if event == "HQ":
    ...
    # bScience_ongoing = True
    # iMax_Science = SCIENCE[sCurrent_Research]
    # iCurrent_Value = 0
    # window['-PROGRESS BAR-'].update(current_count=0, max=iMax_Science)
    # window['-PROGRESS BAR-'].update(visible=True)
    # window['-Cancel_Science-'].update(visible=True)

  elif event == "Erforsche Eisen":
    sCurrent_Research = 'iron'
    do_research(sCurrent_Research)
    window['desc_Research'].update(visible=True, value=SCIENCE['iron']['description'])
    window['desc_duration'].update(visible=True, value=f"Forschungsdauer: {SCIENCE['iron']['duration']} Zyklen")
